...
bool ImageManager::save( const Image& image )
{
	if (CFileIO::fileExists( image.name() ) == true)
	{
		//Log(0) << "ImageManager: File already exists, delete it first\n" << flush;
		return false;
	}

	ImageExporter* ie = this->getExporter( image.name() );
	if (ie != NULL)
	{
		return (*ie)( image.name(), image );
	} 

	//Log(0) << "ImageManager: Couldn't save image. No exporter registered for this type.\n" << flush;
	return false;
}
...
//  returns true or false for successful or unsuccessful
//  writes the Image to it's original filename (Image::name())
bool ImageManager::saveAs( const char* const filename, const Image& image )
{
	if (CFileIO::fileExists( filename ) == true)
	{
		kev::Alert( CFileIO::fileExists( filename ) != true, "ImageManager: File already exists, delete it first" );
		return false;
	}
