...
{
   std::vector<EntityTypePtr>& psList = ps.entities();
   std::vector<EntityTypePtr>::iterator it;
   std::list< std::vector<EntityTypePtr>::iterator > deadList;
   ...
   std::list< std::vector<EntityTypePtr>::iterator >::reverse_iterator deadListIt;
   for (deadListIt = deadList.rbegin(); deadListIt != deadList.rend(); ++deadListIt)
   {
      //ps.remove( (*deadListIt) );

      EntityTypePtr p = *(*deadListIt);
      p->isInSystem( false );
