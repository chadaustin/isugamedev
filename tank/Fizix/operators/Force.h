...
class GlobalOperator : public PsOperator
{
public:
   GlobalOperator() : mIgnoreList() {}
   virtual ~GlobalOperator() {}
   void ignore( Particle* p )
   {
   ...
   bool isIgnored( Particle* p )
   {
//      bool found = false;
      std::vector<Particle*>::iterator it;
      std::list< std::vector<Particle*>::iterator > toKill;
      ...
      if (toKill.size() > 0)
      {
         std::list< std::vector<Particle*>::iterator >::iterator kit;
...
protected:
   std::vector<Particle*> mIgnoreList;
};


//: Binary force, 
// a force operator that acts on only two particles in the system.
class BinaryForce : public PsOperator
{
public:
	BinaryForce(){}
	virtual ~BinaryForce(){}
	...
	virtual void operator()( ParticleSystem& ps ) = 0;
};

//: FORCE
//  A basic force, will apply it's force to all particles in the system.
class GlobalForce : public GlobalOperator
{
public:
        GlobalForce(){}
...
class ViscousDrag : public GlobalForce
{
public:
	ViscousDrag(){}
...
class Acceleration : public GlobalForce
{
public:
	Acceleration() {}
...
class Current : public GlobalForce
{
public:
   Current() : mSpeed()
   {
...
// gravity
class Gravity : public Acceleration
{
public:
   Gravity() : Acceleration( 0.0f, -9.8195f, 0.0f ) {}
   virtual ~Gravity(){}
};


// spring force
class Spring : public BinaryForce
{
public:
