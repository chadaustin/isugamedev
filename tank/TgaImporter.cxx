...
bool TgaImporter::operator()(const char* const filename, Image& image)
{
	...
	*/
	...
	{
		kev::Alert( false && "Error seeking to end of file for possible extension data." );
		return false;
	}
	...
	*/
	...
	{
		kev::Alert( false, "Couldn't seek within the file to the image data portion" );
		return false;
	}
   ...
   //is the image stored upside down?
		...
		{			
         ...
         if (isUpsideDown)
         {
			...
			if (ReadRLERow( image.row( j ), bCount, bytesPerPixel, fp ) < 0)
			{
				...
				kev::Alert( false, "Error decompressing RLE data." );
				//return true;
			}
	...
	else if ( header.imageType == 2 || header.imageType == 3 )
	{
      ...
      // Which row do we start at?
		...
		if ( isUpsideDown )
		{
         cout<<"Upside Down, reading backwards:\n"<<flush;
		...
		if ( isHorzFlipped )
		{
         cout<<"Horz Flipped, reading backwards:\n"<<flush;
		...
		for ( int y = 0; y < image.height(); ++y )
		{
			...
			for ( int x = 0; x < image.width(); ++x )
			{
				...
				if (::fread( image.pixel(i, j), sizeof(unsigned char), onePixelSize, fp ) != onePixelSize)
				{
					...
					// TODO: there may be times when the user will want this to fail, such as a texture loader 
					//       in a professional-grade simulation where you want to ensure your client wont see crap.
					kev::Alert( false, "Error reading image data." );
					return true;
				}
