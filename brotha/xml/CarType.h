...
namespace data {
   ...
   public:
      ...
      CarType(const std::string& name = "none!",
              const std::string& filename = "car.model")
         : mName(name), mFilename(filename)
      {}

      /// Gets the model filename for this car.
      ...
      void xMLify(std::ostream& out) {
         out << "  <cartype name=\"" << mName << "\" file=\"" << mFilename<< "\" />" << std::endl;
      }
