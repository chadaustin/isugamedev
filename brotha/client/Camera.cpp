...
namespace client {
   ...
   void Camera::update(float delta)
   {
      if (mFollow)
      {
         ...
         osg::Quat newRot;
         newRot.slerp(lerpDist, mCurRot, mTargetRot);
         mCurRot = newRot;

         // Lerp to the new position
         osg::Vec3 newPos(mCurPos);
         newPos += ((mTargetPos - mCurPos) * lerpDist);
         mCurPos = newPos;

         // Compute the matrix transform for the camera's lerped target
//         mTargetRot.get(mXForm);
//         mXForm.setTrans(mTargetPos);
         mCurRot.get(mXForm);
         mXForm.setTrans(mCurPos);

         // Take into account the follow distance and pitch
//         osg::Matrix trans, pitchRot;
//         trans = osg::Matrix::translate(0,0,mFollowDist);
//         pitchRot = osg::Matrix::rotate(-mPitch, 1,0,0);

//         mXForm = ( pitchRot * trans) * mXForm;
      }
      else
      {
         mTargetRot.get(mXForm);
   ...
   void Camera::setTarget(const osg::Vec3& pos, const osg::Quat& rot)
   {
      mTargetPos = pos;
      mTargetRot = rot;
   }
