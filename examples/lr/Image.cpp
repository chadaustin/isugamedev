...
{
	unsigned int wide = pixmi::MIN( this->width(),  image.width()  );
	unsigned high = pixmi::MIN( this->height(), image.height() );
...
// flip row j
void        Image::flipRow( int j )
{
   int breadth = this->width();
   int halfdist = (int) ceil( ((double)breadth)/2.0 );
   for (int b = 0; b < halfdist; ++b)
   {
      int swaploc = breadth - b - 1;
      unsigned char temp[4];
      unsigned char* sourcedata = this->pixel( b, j );
      unsigned char* destdata = this->pixel( swaploc, j );
      temp[0] = sourcedata[0];
      temp[1] = sourcedata[1];
      temp[2] = sourcedata[2];

      sourcedata[0] = destdata[0];
      sourcedata[1] = destdata[1];
      sourcedata[2] = destdata[2];

      destdata[0] = temp[0];
      destdata[1] = temp[1];
      destdata[2] = temp[2];

      // handle alpha if there... (HACKY)
      if (this->channels() == 4)
      {
         temp[3] = sourcedata[3];
         sourcedata[3] = destdata[3];
         destdata[3] = temp[3];
      }               
   }
}
...
// flip column j
void        Image::flipColumn( int j )
{
   int breadth = this->height();
   int halfdist = (int)ceil( ((double)breadth)/2.0f );
   for (int b = 0; b < halfdist; ++b)
   {
      int swaploc = breadth - b - 1;
      unsigned char temp[4];
      unsigned char* sourcedata = this->pixel( j, b );
      unsigned char* destdata = this->pixel( j, swaploc );
      temp[0] = sourcedata[0];
      temp[1] = sourcedata[1];
      temp[2] = sourcedata[2];
