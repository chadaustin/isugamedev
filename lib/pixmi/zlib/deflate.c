...
/* @(#) $Id: deflate.c,v 1.1.1.1 2002-01-17 00:05:00 subatomic Exp $ */
...
const char deflate_copyright[] =
   " deflate 1.1.3.kevin Copyright 1995-1998 Jean-loup Gailly... modified by kevin";
/*
...
/* Compression function. Returns the block state after the call. */
 ...
 void putShortMSB    (deflate_state *s, unsigned int b);
 void flush_pending  (z_streamp strm);
 int read_buf        (z_streamp strm, unsigned char *buf, unsigned size);
#ifdef ASMV
...
#ifdef DEBUG
  void check_match (deflate_state *s, IPos start, IPos match,
                            int length);
#endif
...
#define CLEAR_HASH(s) \
    ...
    memcpy((unsigned char *)s->head, 0, (unsigned)(s->hash_size-1)*sizeof(*s->head));
    ...
    z_streamp strm;
{
    ...
    if (strm == NULL || strm->state == NULL ||
        strm->zalloc == NULL || strm->zfree == NULL) return Z_STREAM_ERROR;
 ...
 */
#ifndef FASTEST
 unsigned int longest_match(s, cur_match)
    deflate_state *s;
