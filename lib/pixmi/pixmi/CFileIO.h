...
#include "Endian.h" //needed for pixmi::isBigEndian/pixmi::isLittleEndian funcs
...
namespace CFileIO
{
   ...
   inline static int ReadData( Endianness fileByteOrdering, FILE* fp, typeT& data )
   {
      ...
      if (fileByteOrdering == LITTLE && pixmi::isBigEndian() ||
         fileByteOrdering == BIG && pixmi::isLittleEndian())
      {
         pixmi::byteReverse( data );
      }
