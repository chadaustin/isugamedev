...
bool SgiImporter::load( const char* const filename, Image& image )
{
...
bool SgiImporter::LoadRLE(ifstream &file, Image& image)
{
    ...
    for (y = 0; y < scanlines; y++)
    {
      ...
      if (pixmi::isLittleEndian())
      {
         pixmi::byteReverse(startTable[y]);
      }
    }
    ...
    for (y = 0; y < scanlines; y++)
    {
       ...
       if (pixmi::isLittleEndian())
       {
          pixmi::byteReverse(lengthTable[y]);
       }
